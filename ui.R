...
ui <-  navbarPage("ObseRtools",
                  ...
                  ### Register button ----
                  navbarMenu("Register",
                             tabPanel("Define ad hoc Observation Instrument",
                                      sidebarLayout(
                                        sidebarPanel(
                                          width = 3,
                                          h3(em(strong("Observation Instrument"))),
                                          br(),
                                          selectInput(
                                            "register",
                                            ...
                                            choices = c(
                                              ...
                                              "Field Format" = "Field",
                                              "Category System +  Field Format" = "Combine"
                                            ),
                                          ),
                                          conditionalPanel(
                                            condition = "input.register == 'Field' || input.register == 'Combine' " ,
                                            numericInput(
                                            "dimensions",
                                            label = "Dimensions",
                                            min = 1, value = 1, step = 1, width = 75
                                          )),
                                          
                                          textInput("caption_1", "Dimensión 1", "Indique el nombre de la dimensión 1"),
                                          
                                          conditionalPanel(
                                            condition = "input.register == 'Field' || input.register == 'Combine' || input.register == 'Category'" ,
                                            numericInput(
                                              "categories",
                                              label = "Categories",
                                              min = 1, value = 1, step = 1, width = 75
                                          ...
                                          conditionalPanel(
                                            ...
                                            textInput("caption_2", "Dimensión 2", "Indique el nombre de la dimensión 2")),
                                          
                                          conditionalPanel(
                                            condition = "input.dimensions >= 3" ,
                                            textInput("caption_3", "Dimensión 3", "Indique el nombre de la dimensión 3")),
                                          
                                          uiOutput("dimensiones"),
    ...
    mainPanel(
      tabsetPanel(
        tabPanel(
              ...
              conditionalPanel(
                              condition = "input.dimensions == 2",
                              verbatimTextOutput("dimension_2")
                              ),
              
                conditionalPanel(
                condition = "input.categories >= 1",
                verbatimTextOutput("caption_")
                ),
        ...
        tabPanel(
              h4("Register"),
              actionButton("start", "Start", width = 100, class = "btn btn-success"), #https://getbootstrap.com/docs/4.0/components/buttons/
              HTML("<br/>"),
              tags$br(),
              actionButton("end", "End", width = 100, class = "btn-warning"),
              HTML("<br/>"),
              tags$br(),
    ...
    navbarMenu("Observers Agreement",
               tabPanel("Bangdiwala Agreement Chart",
                        sidebarLayout(
                          sidebarPanel(title = "Agreement Data",
                            fileInput('table_agreement', 'Select HOISAN Quality Control output file',
                            accept=c(".xlsx",".xls",'dltext/csv','text/comma-separated-values,text/plain','.csv')),
                            radioButtons("table_type", "Data Format", choices = c("xls", "csv"), selected = "xls",inline=T),
                            radioButtons("data_type", "Data Type", choices = c("Wide Data", "Contingency Table"), selected = "Wide Data",inline=T)
                                      ),
                          mainPanel(
                            tabsetPanel(
                            tabPanel("Agreement Data", shiny::fluidRow(
                              column(width = 6, DTOutput("bang_data")),
                              column(width = 6, DTOutput("bang_table")))),
                            tabPanel("Bangdiwala Agreement Chart", shiny::fluidRow(
                              column(width = 6, plotOutput("bang_plot",click = "plot_click"),height="auto"),
                              column(width = 6, DTOutput("bang_res"), 
                                                DTOutput("kappa_res"), 
                                                DTOutput("kripp_res"),
                                                DTOutput("gwet_res"))))
                            ))))
